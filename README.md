...
Twenty-four roadmaps (**v1–v24**, Phases 0–134) have shipped and are merged to `main`, each green on a cleared clean build — 6 unit suites plus the full smoke / fuzz aggregate, **JIT and AOT**, on ubuntu + macOS CI. Current release: **[v0.24.0](https://github.com/kardashevlang/kardashev/releases/latest)**.
...
**v24 — diagnostics & the developer surface (shipped):** the highest-ROI gap on the road to production. Errors became real, navigable **diagnostics** — a source snippet with a caret and the user's own line number (the ~450-line prelude offset is recovered) — plus an opt-in **lint** (`kardc -W`: unused vars + unreachable code, sound/no-false-positives), **error codes** with `kardc --explain Exxxx`, **`///` doc comments** (surfaced by the formatter and LSP hover), and parser **panic-mode recovery** (one diagnostic per error, no cascade). **Next (v25+) — and an honest take on where this stands vs production languages:** see **[ROADMAP.md](ROADMAP.md)**.
