...
> **Status: planned, not yet implemented.** The phases below are the *intended* v7 (Phases 39–44). As in v1–v6, each phase ships fully green (6 unit suites + the smoke-test aggregate, JIT **and** AOT) before the next builds on it. Statuses are a **planned marker (🎯)**, not the shipped checkmark.

v6 made the heap recursive and proved it on a full nested-JSON value — but that capstone could only carry **integers** (`1.5` doesn't even lex) and its object/array helpers were hand-written because kardashev still has **no generic `impl<T>` blocks** (`impl<T: Clone> Clone for Vec<T>` is a parse error). v7's theme is **real numbers, real abstraction**: give the value layer `f64`, give the abstraction layer generic impls so `Clone`/`Display`/`Eq` become real traits you can `#[derive]`, then prove it by upgrading JSON to carry floats + escapes with derived helpers — still leak-free under the constant-memory gate.

| Phase | Goal | Status |
|-------|------|--------|
| 39 | **`f64` floating-point** | 🎯 *Planned.* Lexer float literals (`1.5`, `-2.0`, `3e8`, `1.0e-3`); an `f64` type lowering to LLVM `double`; arithmetic (`+ - * /`), comparisons, and `f64`↔`i64` conversions (`to_f64` / `trunc`); `f64` params/returns/`let`s; printing. JIT entry/AOT exit stay i64, so an `f64` program returns via an i64 cast or prints. **Accept:** a numeric kernel over `f64` (sum/avg of a `Vec<f64>`, a comparison, an int↔float round trip) computes correctly JIT + AOT. *(Single float width — no `f32`; NaN/Inf are whatever LLVM does, not specially handled.)* |
| 40 | **Generic `impl<T: Bound>` blocks** | 🎯 *Planned.* Parse + typecheck + monomorphize `impl<T: Trait> Trait2 for Container<T>` (the open Phase-21 gap): a generic impl header with its own type params + bounds, resolved per concrete instantiation through the existing monomorphization worklist. **Accept:** a user `struct Pair<T> { ... }` with `impl<T: Display> Display for Pair<T>` formats `Pair<i64>` and `Pair<String>`; a `impl<T: Eq> Eq for Vec<T>` compiles and is callable — JIT + AOT. |
| 41 | **Real generic `Clone` / `Eq` over containers** | 🎯 *Planned.* With 40 in hand, ship prelude `impl<T: Clone> Clone for Vec<T>` / `Box<T>` and `impl<K,V: Clone> Clone for HashMap<K,V>` (and `Eq` likewise), so cloning/comparing dispatches through the element's own impl rather than the Phase-35 structural intrinsic — the intrinsic stays as the fallback for types with no impl. **Accept:** a `Vec<Pair<String>>` deep-clones via the user `Clone for Pair`, and two such vecs compare via `Eq` — constant-memory, JIT + AOT. |
| 42 | **`#[derive(Clone, Eq, Display)]`** | 🎯 *Planned.* An attribute on a `struct`/`enum` that synthesizes the obvious impls (field-/payload-wise) at typecheck time, reusing 40–41's machinery — no hand-written boilerplate. **Accept:** `#[derive(Clone, Eq)] enum Json { ... }` clones + compares deeply with no hand-written impl; `#[derive(Display)] struct P { x: i64, y: i64 }` prints `P { x: .., y: .. }` — JIT + AOT. |
| 43 | **Runtime string escapes + close the async-frame leak** | 🎯 *Planned.* `str_unescape`/`str_escape` decode/encode `\n \t \" \\ \/ \r \uXXXX` for RUNTIME-parsed strings (source-literal escapes already lex); and free a completed `Future`'s heap frame at the executor task-lifecycle boundary (the last documented leak, codegen.cpp `inAsyncFn_` site). **Accept:** a round-trip of `"line\nbreak\t\"q\""` through parse→serialize is byte-exact; an async program that completes N futures holds flat peak-RSS — JIT + AOT. |
| 44 | **Capstone: JSON 2.0 — floats + escapes + derived helpers** | 🎯 *Planned.* Upgrade `examples/json` to a `JNum(f64)` (replacing/augmenting `JInt`) with escape-aware string parse/serialize, and `#[derive(Clone, Eq)]` on `Json` (retiring the hand-written `json_eq` + showing derive end to end). **Accept:** parse `{"pi":3.14159,"msg":"a\nb","xs":[1.5,-2.0,true,null]}`, round-trip by derived `Eq`, constant peak-RSS across a parse-and-drop loop, JIT + AOT, wired into `make -f Makefile.local test`. **The north star.** |

**Intended landing order — 39 → 40 → 41 → 42 → 43 → 44.** 39 adds the missing scalar (lowest risk, self-contained). 40 is the **keystone**: generic `impl<T>` blocks unblock everything abstraction-related. 41 uses 40 to make `Clone`/`Eq` real generic traits; 42 derives them automatically. 43 closes the runtime-escape + async-frame gaps the capstone needs. 44 integrates all five into a floats-and-escapes JSON with derived helpers — the integration proof under its constant-memory gate. Each phase ships fully green before the next, exactly as v1–v6 did.
