...
> **Status: planned, not yet implemented.** The phases below are the *intended* v9 (Phases 51–56). As in v1–v8, each phase ships fully green (6 unit suites + the smoke-test aggregate, JIT **and** AOT) before the next builds on it. Statuses are a **planned marker (🎯)**, not the shipped checkmark.

v8 finished the generics core — bounded container ops, HashMap trait impls, `Ord`/`sort`, the full `#[derive]` set, associated functions, and generic trait objects — and proved it with a HashMap-backed, fully-derived JSON 3.0. But the stdlib still has no way to *transform* a collection without a hand-written `while` loop, two deferrals remain (trait `Clone`/`Eq` for `Box`; **generic** associated functions), and there is no string tokenizer. v9's theme is **data in motion**: close those two deferrals, give `Vec` real higher-order combinators (`map`/`filter`/`fold` over closures), add string splitting, and prove it with a capstone that is a genuine data pipeline — a word-frequency histogram read from text, counted in a `HashMap`, and ranked by a derived `Ord`.

| Phase | Goal | Status |
|-------|------|--------|
| 51 | **`Box<T>` as a first-class impl target + deref ergonomics** | 🎯 *Planned.* Make `impl Trait for Box<T>` registrable (Box gets a `typeName` in impl registration + method dispatch) and parse/typecheck `&*e` / `**e` so a `&Box<T>` body can reach `&T`. Ship prelude `impl<T: Clone> Clone for Box<T>` + `impl<T: Eq> Eq for Box<T>` — closing the v8 deferral. **Accept:** a `Box<i64>` and a `Box<String>` clone via `.clone()` and compare via `.eq()`; a `#[derive(Clone, Eq)]` type with a `Box<T>` field round-trips — JIT + AOT. |
| 52 | **Generic associated functions** | 🎯 *Planned.* Extend Phase-48's `Type::method()` static-call resolution to a *bounded generic* `T::method()` (e.g. `T::default()` for `<T: Default>`), monomorphized per instance like a bounded-generic method call. **Accept:** a `fn pair_of<T: Default>() -> (T, T) { (T::default(), T::default()) }` runs for `i64` and `String`; a generic `#[derive(Default)] struct Pair<T>` constructs via `Pair::<i64>::default()`-style use — JIT + AOT. |
| 53 | **`Vec` higher-order combinators** | 🎯 *Planned.* Generic `vec_map<T, U>(&Vec<T>, fn(&T) -> U) -> Vec<U>`, `vec_filter<T>(&Vec<T>, fn(&T) -> bool) -> Vec<T>`, and `vec_fold<T, A>(&Vec<T>, A, fn(A, &T) -> A) -> A` — over the existing closure machinery, returning fresh owned Vecs (the borrow-only-reads contract keeps them leak-free). **Accept:** map a `Vec<i64>` to a `Vec<String>`, filter evens, fold a sum — each via a closure, JIT + AOT, constant-memory over a loop. |
| 54 | **String tokenizing** | 🎯 *Planned.* `str_split(&String, sep: i64) -> Vec<String>` (split on a byte) + `str_trim` + `str_eq`-based helpers, written in kardashev over `str_char_at`/`str_substring`/`str_push_byte`. **Accept:** split `"a,bb,,c"` on `,` into 4 pieces (incl. the empty one), trim `"  hi  "` → `"hi"`, JIT + AOT, leak-free. |
| 55 | **HashMap entry iteration → `Vec<(K, V)>`** | 🎯 *Planned.* `hashmap_entries<K, V>(&HashMap<K,V>) -> Vec<(K, V)>` (deep-cloned key/value pairs as tuples) so a map can be ranked/printed without manual key lookups, building on Phase-36 non-Copy tuples + Phase-38 `hashmap_keys`. **Accept:** build a `HashMap<String, i64>`, take its entries as a `Vec<(String, i64)>`, sort by a derived-`Ord` wrapper, read back — JIT + AOT, constant-memory. |
| 56 | **Capstone: word-frequency histogram** | 🎯 *Planned.* A real pipeline written in kardashev: tokenize a text (`str_split`), count words in a `HashMap<String, i64>` (`vec_fold`), turn the map into ranked `Vec<Count>` where `#[derive(Ord, Eq)] struct Count { n: i64, word: String }` orders by count-then-word (`sort`), and print the top entries. **Accept:** over a fixed paragraph, produce the correct top-3 words + counts, deterministic (sorted) output, constant peak-RSS across a re-run loop, JIT + AOT, wired into `make -f Makefile.local test`. **A data pipeline, end to end, in kardashev.** |

**Landing order — 51 → 52 → 53 → 54 → 55 → 56.** 51 and 52 clear the two v8 deferrals (Box trait impls; generic associated functions). 53 adds the `Vec` combinators the pipeline transforms with. 54 adds the tokenizer that feeds it. 55 makes a `HashMap` rankable as entries. 56 integrates all of it — split → fold-count → entries → derived-`Ord` sort → top-N — the data pipeline. Each phase ships fully green before the next, as v1–v8 did.
